...
class Dataset_ETT_hour(Dataset):
    def __init__(
        ...
        # Ensure data_path ends with .csv
    ...
    def __read_data__(self):
        ...
        if self.timeenc == 0:
            df_stamp["year"] = df_stamp.date.apply(lambda row: row.year, 1)  # 新加
    ...
    def __getitem__(self, index):
        ...
        seq_y = self.data_y[r_begin:r_end, :]          # [label_len+pred_len, N]
        ...
        with h5py.File(file_path, 'r') as hf:
            emb = hf['embeddings'][:]
        embeddings = torch.from_numpy(emb).squeeze()        # (768,)
