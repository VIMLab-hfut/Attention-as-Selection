...
from utils.toolsadd import StandardScaler
...
class Dataset_ETT_hour(Dataset):
    def __init__(self, root_path="./datasets/ETT-small/", flag='train', size=None, 
        ...
        self.embed_path = f"./Embeddings/{data_path}/{flag}/"
    ...
    def __read_data__(self):
        ...
        if not self.data_path.endswith('.csv'):
            self.data_path = self.data_path + ".csv"

        # Combine once, no repetition
        ...
        # ===============================================
        self.batch_files = sorted([os.path.join(self.embed_path, f)
                                for f in os.listdir(self.embed_path) if f.endswith('.h5')])
        self.idx_to_batch = []  # (batch_file, local_idx_in_batch)
        for bf in self.batch_files:
            with h5py.File(bf, 'r') as hf:
                n = hf['embeddings'].shape[0]  
                self.idx_to_batch.extend([(bf, i) for i in range(n)])
    ...
    def __getitem__(self, index):
        ...
        batch_file, local_idx = self.idx_to_batch[index]
        with h5py.File(batch_file, 'r') as hf:
            emb = hf['embeddings'][local_idx]  # [token_len, d_model]
        

        target_shape = (62, 768)  
        emb = self._process_embedding_shape(emb, target_shape)
        
        embeddings = torch.from_numpy(emb)  # [token_len, d_model]

        return seq_x, seq_y, seq_x_mark, seq_y_mark, embeddings

    def _process_embedding_shape(self, emb, target_shape):
        ...
        if current_shape[0] < target_shape[0]:
            ...
            emb = np.pad(emb, pad_width, mode='constant', constant_values=0)
    ...
    def __len__(self):
        return len(self.idx_to_batch)

    def inverse_transform(self, data):
        return self.scaler.inverse_transform(data)
   
class Dataset_ETT_minute(Dataset):
    def __init__(self, root_path="./datasets/", flag='train', size=None, 
