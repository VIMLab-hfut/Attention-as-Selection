...
def vali(
    ...
    model.eval()
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,embeddings) in tqdm(
            ...
            if args.use_amp:
                with torch.cuda.amp.autocast():
                    if args.output_attention:
                        outputs,_ = model(batch_x, batch_x_mark, dec_inp, batch_y_mark,embeddings)[0]
                    else:
                        outputs,_ = model(batch_x, batch_x_mark, dec_inp, batch_y_mark,embeddings)
            else:
                if args.output_attention:
                    outputs,_ = model(batch_x, batch_x_mark, dec_inp, batch_y_mark,embeddings)[0]
                else:
...
def test(
    args, accelerator, model, train_loader, vali_loader, criterion, epoch, mode="vali"
):
    ...
    with torch.no_grad():
        ...
        for i in range(len(id_list) - 1):
            outputs[id_list[i] : id_list[i + 1], :, :],_ = model(
                x[id_list[i] : id_list[i + 1]],
                None,
                dec_inp[id_list[i] : id_list[i + 1]],
                None,
                embeddings=None,        # test 不需要 embeddings 的话
                batch_y=y[id_list[i]:id_list[i+1]],
                epoch=epoch,
                batch=i,
                mode=mode
            )
