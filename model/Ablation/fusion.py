...
from layers.Cross_Modal_Align import CrossModalFusion
...
class LearnableTemperatureInfoNCELoss(nn.Module):
    ...
    def forward(self, query, positive, negative=None):
        ...
        query = query.mean(dim=1)#nfoNCE Loss 使用 mean pooling → 对能量敏感
...
class Model(nn.Module):
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        # Requires PyTorch >=1.10 for batch_first=True; 若低版本可把维度转置再用
        self.cross_t2s = CrossModalFusion(d_model=768, n_heads=8)
        self.cross_s2t =CrossModalFusion(d_model=768, n_heads=8)

        self.cross_ln_text = nn.LayerNorm(self.d_llm)
        self.cross_ln_time = nn.LayerNorm(self.d_llm)

        # 一个小的可学习融合权重（保留你已有的 gamma 也行）
...
# 方案一：Q_time to KVtext
    def forecast(  
        ...
        # print(x_enc.shape)#([24, 512, 1])
        x_enc = self.normalize_layers(x_enc, "norm")  
        # print(x_enc.shape)#torch.Size([24, 512, 1])
        B, T, N = x_enc.size()
        x_enc = x_enc.permute(0, 2, 1).contiguous()
        # print(x_enc.shape)#torch.Size([24, 1, 512])
        enc_out, n_vars = self.patch_embedding(x_enc.to(torch.bfloat16))          # (B, L, d_model)

        # ---------- 直接用 DataLoader 给的 embedding ----------
        # ---------- 1. 把 (B*N, 768) 展开成 (B*N, prompt_token_len, llm_dim) ----------
        prompt_embeddings = embeddings.to(x_enc.device)                       
        # print("prompt_embeddings.shape",prompt_embeddings.shape)#torch.Size([24, 768])
        # === 关键修正：确保与原始尺寸一致 ===
        prompt_embeddings = prompt_embeddings.unsqueeze(1).repeat(1, N, 1)   # (B, N, 768)
        prompt_embeddings = prompt_embeddings.reshape(B * N, -1)            # (B*N, 768)

        # ====== 与原逻辑完全一致 ======
        B_N = prompt_embeddings.size(0)

        prompt_seq = self.prompt_expander(prompt_embeddings)                 # (B*N, prompt_len*768)有风险 巨大的线性映射训练过大的映射矩阵
        prompt_seq = prompt_seq.view(B_N, self.prompt_len, self.d_llm)    # (B*N, prompt_len, 768)
        # print("prompt_seq.shape",prompt_seq.shape)
        Learn_prompt = self.learnable_prompt.unsqueeze(0).expand(B_N, -1, -1)#没有梯度裁剪（clip_grad_norm_），训练跨注意力结构很容易爆炸
        # print("Learn_prompt.shape",Learn_prompt.shape)
        prompt_embeddings_all = torch.cat([Learn_prompt, prompt_seq], dim=1)
        TS = enc_out
        TS = self.tsprojection(TS)
        llama_inputs = prompt_embeddings_all # [B*N, prompt_total, d_llm]
        llm_out = self.llm_model(inputs_embeds=llama_inputs)
        llm_hidden_states = llm_out.last_hidden_state   # [B*N, prompt_total, d_llm]

        # keep only the last prompt_len tokens (same as original)
        dec_out = llm_hidden_states[:, -self.prompt_len :, :]  # [B*N, prompt_len, d_llm]
        time_attn_out, attn = self.cross_t2s(TS, dec_out)  # [B*N, prompt_total, d_llm]
        # print("time_attn_out",time_attn_out)
       

    
        TS_fused = time_attn_out

        infonce_loss = self.LearnableTemperatureInfoNCELoss(TS_fused, dec_out)
     

        alignment_loss = infonce_loss 


        dec_out = torch.cat(
            (dec_out[:, :, : self.d_ff], TS_fused[:, :, : self.d_ff]), dim=1#拼接TS_fused
        )
